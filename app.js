...
allowedPartyTypes = ["fire", "water", "electric"];

pokemon.forEach((pokemon) => {
    if (pokemon.starter && allowedPartyTypes.includes(pokemon.type) && game.party.length < 4 && !game.party.includes(pokemon)) {
...
pokemon.forEach((pokemon) => {
    if (pokemon.name === "Ivysaur") {
        game.party.splice(0, 1, pokemon);
    }
    if (pokemon.name === "Charmeleon") {
        game.party.splice(1, 1, pokemon);
    }
    if (pokemon.name === "Wartortle") {
        game.party.splice(2, 1, pokemon);
    }
    if (pokemon.name === "Raichu") {
        game.party.splice(3, 1, pokemon);
    }
});
...
game.catchPokemon(pokemon[pokemon.length - 2]); // should be Mewtwo
...
game.catchPokemon(pokemon[pokemon.length - 1]); // should be Mew
...
game.catchPokemon(pokemon[pokemon.length - 3]); // should be Dragonite
...
game.catchPokemon(pokemon[129]); // should be Gyarados
...
// catch Aerodactyl
game.catchPokemon('Aerodactyl');
console.log('Exercise 20: ',game.items[1]);
